...
describe 'Visitant see all ratings' do
  it 'successfully' do
    #arrange
    ...
    #assert
    expect(current_path).to eq buffet_ratings_path(buffet.id)
    within("div#ratings") do
      ...
      expect(page).to have_content "Por: Jules"
      expect(page).to have_content "muito legal 2"

      expect(page).to have_content "3/5"
      expect(page).to have_content "Por: Rafa"
      expect(page).to have_content "muito legal 3"

      expect(page).to have_content "2/5"
      expect(page).to have_content "Por: João"
      expect(page).to have_content "muito legal 4"
    end
  end

  it "and cant see all ratings page if buffet ratings are equal or less than 3" do
    #arrange
    company = Company.create!(
      buffet_name: "Buffet Legal",
      company_registration_number: "74.391.888/0001-77",
    ...
    visit buffet_path(buffet.id)
    #assert
    expect(page).not_to have_link "Ver todas as avaliações"
  end
end
